...
if [ -n "$PHP_CLI_MEMORY_LIMIT" ]; then
	...
	sed -i "s/memory_limit = .*/memory_limit = $PHP_CLI_MEMORY_LIMIT/" /etc/php/7.0/cli/php.ini
fi;
if [ -n "$PHP_FPM_MEMORY_LIMIT" ]; then
    echo "Set fpm memory limit: $PHP_FPM_MEMORY_LIMIT";
    sed -i "s/memory_limit = .*/memory_limit = $PHP_FPM_MEMORY_LIMIT/" /etc/php/7.0/fpm/php.ini
fi;
...
if [ "$PHP_XDEBUG_REMOTE_AUTOSTART" = "1" ]; then
    sed -i "s/xdebug.remote_autostart=0/xdebug.remote_autostart=1/" /etc/php/7.0/mods-available/xdebug.ini
    sed -i "s/xdebug.remote_connect_back=.*/xdebug.remote_host=$HOST_IP/" /etc/php/7.0/mods-available/xdebug.ini
